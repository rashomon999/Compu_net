// ============================================
// js/generated/ChatSystem.js - IDEMPOTENTE
// ============================================
...
if (!Ice) {
  ...
  if (!window.Ice.ChatSystem) {
    console.log('✅ Ice.js disponible, inicializando ChatSystem...');
    initializeChatSystem(Ice);
  } else {
    console.log('⚠️ ChatSystem ya está inicializado, reutilizando...');
  }
}
...
function initializeChatSystem(Ice) {
  ...
  // ✅ CRÍTICO: Verificar si ya existe
  let ChatSystem = _ModuleRegistry.module("ChatSystem");
  
  // ✅ Si ya existe, no redefinir
  if (ChatSystem.Message) {
    console.log('⚠️ ChatSystem ya definido, saltando inicialización');
    window.Ice.ChatSystem = ChatSystem;
    return;
  }

  console.log('🔧 Definiendo ChatSystem por primera vez...');
  ...
  // ✅ Definir sequences solo si no existen
  if (!ChatSystem.MessageSeqHelper) {
    Slice.defineSequence(ChatSystem, "MessageSeqHelper", "ChatSystem.Message", false);
  }
  if (!ChatSystem.StringSeqHelper) {
  ...
  if (!ChatSystem.VoiceNoteSeqHelper) {
  ...
  if (!ChatSystem.NotificationCallback) {
  ...
  if (!ChatSystem.NotificationService) {
  ...
  console.log('✅ ChatSystem.js inicializado correctamente');
...
// ✅ EXPORTAR para que Webpack pueda importarlo
export default {};