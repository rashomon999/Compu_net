...
const initChatSystem = () => {
    ...
    if (!ChatSystem.CallOfferSeqHelper) {
    ...
    if (!ChatSystem.CallAnswerSeqHelper) {
    ...
    if (!ChatSystem.RtcCandidateSeqHelper) {
        Slice.defineSequence(ChatSystem, "RtcCandidateSeqHelper", "ChatSystem.RtcCandidate", false);
    }

    ChatSystem.AudioChunk = class {
        constructor(data = null, timestamp = new Ice.Long(0, 0)) {
            this.data = data;
            this.timestamp = timestamp;
        }

        _write(ostr) {
            ChatSystem.BytesHelper.write(ostr, this.data);
            ostr.writeLong(this.timestamp);
        }

        _read(istr) {
            this.data = ChatSystem.BytesHelper.read(istr);
            this.timestamp = istr.readLong();
        }
    ...
    Slice.defineStruct(ChatSystem.AudioChunk, true, true);
    ...
    Slice.defineOperations(ChatSystem.CallCallback, ChatSystem.CallCallbackPrx, iceC_ChatSystem_CallCallback_ids, 0, {
        ...
        "onCallEnded": [, , , , , [[7], [7]], , , ,],
        "onAudioChunk": [, , , , , [[ChatSystem.AudioChunk]], , , ,]
    });
